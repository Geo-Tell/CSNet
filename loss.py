...
class CE(nn.Module):
    def __init__(self, ignore_lb=255, weight=None, *args, **kwargs):
        ...
        self.criteria = nn.CrossEntropyLoss(ignore_index=ignore_lb, weight=weight)
    def forward(self, logits, labels):
        logits_shape = logits.size()[2:]
        labels_shape = labels.size()[1:]
        if not (logits_shape == labels_shape):
            labels = F.interpolate(labels.float(), (H, W), mode='nearest')
        return self.criteria(logits, labels)


class OhemCELoss(nn.Module):
    def __init__(self, ignore_label=255, thresh=0.7, min_kept=100000, factor=8, weight=None):
        ...
        self.factor = factor
        self.criterion = torch.nn.CrossEntropyLoss(ignore_index=ignore_label, weight=weight)
