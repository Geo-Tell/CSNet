...
class Bottleneck(nn.Module):
    def __init__(self,
        ...
        self.conv3 = nn.Conv2d(mid_chan,
                ...
                kernel_size=1,
                bias=False)
        self.bn3 = BatchNorm2d(out_chan, activation='identity')
        self.relu = nn.ReLU()
        ...
        if in_chan != out_chan or stride != 1:
            self.downsample = nn.Sequential(
                ...
                BatchNorm2d(out_chan, activation='identity'))
        # self.init_weight()
...
class Resnet101(nn.Module):
    ...
    def init_weight(self):
        state_dict = torch.load('./resnet101.pth', map_location=torch.device('cpu'))
        ...
        for k, v in self_state_dict.items():
            if k in state_dict.keys():
                ...
                if k.startswith('conv1'):
                    v = torch.cat([state_dict[mk], torch.mean(state_dict[mk], 1).unsqueeze(1)], 1)
...
class Resnet50(nn.Module):
    ...
    def init_weight(self):
        ...
        state_dict = torch.load('/home/hlx/MyResearch/Segmentation/pretrained/resnet50.pth')
